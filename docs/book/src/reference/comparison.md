...
| **WiFi AP management** | UniFi controllers + TP-Link EAP standalone | Native (runs on the AP) | Not built-in | Not applicable (wired only) | UniFi APs only | Native (hostapd, the device IS the AP) | Not built-in |
...
**WiFi AP support covers two vendors.** UniFi controllers (OS and legacy, including UDM, UDR, Cloud Key) and TP-Link EAP standalone APs are supported with full SSID management, radio configuration, and client visibility. Other AP brands work fine on the network but are managed through their own interfaces. OpenWrt runs directly on APs. Ubiquiti has a deeper native AP ecosystem. RaspAP turns a Raspberry Pi into a WiFi AP via hostapd.
...
**Install script requires Debian/Ubuntu.** The `install.sh` script and APT repo target Debian 12+, Ubuntu 22.04+, and Raspbian. However, the static musl binaries run on any Linux distro, and the Docker all-in-one container (Alpine-based) works anywhere Docker runs. OPNsense is FreeBSD-only; VyOS is Debian-based with its own installer; RaspAP runs on Raspberry Pi OS / Debian; Firewalla and Ubiquiti ship their own firmware.
...
- You're comfortable with Linux and a CLI or Docker install process

## When to Choose Something Else

- You need plug-and-play with a mobile app (Firewalla)
- You want deeper AP management than UniFi SSID/radio/client control (Ubiquiti native)
