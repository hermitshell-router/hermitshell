...
Query logs from Unbound reveal browsing activity for every device on the network. Exported via syslog (UDP, unencrypted — SECURITY.md #53).
...
**Mitigated by:** Per-device /32 nftables isolation, group-based forwarding chains, MAC-IP validation (SECURITY.md #84), static ARP/NDP binding (#83), DHCP fingerprint change detection (#85), UPnP group filtering (#81), mDNS group filtering (#73).
...
**Mitigated by:** Docker `--cap-drop ALL --read-only --security-opt no-new-privileges` (standalone mode), SO_PEERCRED method allowlist (#90), DHCP/DNS-log methods excluded from web-allowed set. The allowlist is acknowledged as permissive (SECURITY.md #90).
...
**Mitigated by:** Limited — secrets at rest are not encrypted (SECURITY.md #36, #51, #52). Physical access is considered full compromise. The setup wizard's unauthenticated window (#60) is only exploitable at first boot, which assumes physical control.
...
**Mitigated by:** HTTPS for GitHub API and downloads. SHA256 checksum verification on tarballs (integrity, not authenticity). **No GPG signature verification** (SECURITY.md #86). Auto-update amplifies this risk (#87). Tarball extraction does not reject symlinks (#102).
...
**Mitigated by:** Transaction ID matching (`xid`), `dhcproto` protocol validation. DHCP inherently trusts the server — this is accepted risk (SECURITY.md #31).
...
**Residual risk:** The web-allowed set is broad (SECURITY.md #90). `apply_config` bypasses `BLOCKED_CONFIG_KEYS` (#110).
...
**Threats:** DNS cache poisoning (mitigated by DNSSEC + upstream DoT/DoH), DNS tunneling for data exfiltration, DNS rebinding attacks (mitigated by `private-address` directives, #100). Blocklist downloads now require HTTPS and reject internal IPs (SECURITY.md #142, fixes #92).
...
The following risks are documented in SECURITY.md and explicitly accepted as part of the threat model:
...
| ~~91~~ | ~~DHCP IPC socket lacks SO_PEERCRED~~ | Fixed: now enforces `SO_PEERCRED` UID 0 check (SECURITY.md #131) |
| ~~101~~ | ~~`thread_rng` for session secret~~ | Fixed: replaced with `OsRng` (SECURITY.md #134) |
