...
/// Validate WireGuard public key: base64-encoded, exactly 44 characters.
fn validate_pubkey(key: &str) -> Result<()> {
    ...
    let output = Command::new("base64")
        ...
        .spawn()
        .and_then(|mut child| {
            child.stdin.take().unwrap().write_all(key.as_bytes())?;
            child.wait()
        })?;
    if !output.success() {
        anyhow::bail!("invalid base64 in WireGuard public key");
    }
    Ok(())
}
