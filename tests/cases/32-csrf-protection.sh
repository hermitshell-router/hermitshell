...
response=$(vm_exec router "curl -s -k -o /dev/null -w '%{http_code}' -X POST -d 'password=testpass123' https://localhost${login_action}")
...
response=$(vm_exec router "curl -s -k -o /dev/null -w '%{http_code}' -H 'Sec-Fetch-Site: same-origin' -X POST -d 'password=testpass123' https://localhost${login_action}")
...
response=$(vm_exec router "curl -s -k -o /dev/null -w '%{http_code}' -H 'Sec-Fetch-Site: cross-site' -X POST -d 'password=testpass123' https://localhost${login_action}")
...
response=$(vm_exec router "curl -s -k -o /dev/null -w '%{http_code}' -H 'Sec-Fetch-Site: same-site' -X POST -d 'password=testpass123' https://localhost${login_action}")
assert_match "$response" "403" "POST with Sec-Fetch-Site: same-site blocked"

# POST with Sec-Fetch-Site: none should be rejected
response=$(vm_exec router "curl -s -k -o /dev/null -w '%{http_code}' -H 'Sec-Fetch-Site: none' -X POST -d 'password=testpass123' https://localhost${login_action}")
assert_match "$response" "403" "POST with Sec-Fetch-Site: none blocked"

# POST with matching Origin header should succeed
response=$(vm_exec router "curl -s -k -o /dev/null -w '%{http_code}' -H 'Origin: https://localhost' -X POST -d 'password=testpass123' https://localhost${login_action}")
assert_match "$response" "200|30[0-9]" "POST with matching Origin succeeds"

# POST with mismatched Origin header should be rejected
response=$(vm_exec router "curl -s -k -o /dev/null -w '%{http_code}' -H 'Origin: https://evil.com' -X POST -d 'password=testpass123' https://localhost${login_action}")
assert_match "$response" "403" "POST with mismatched Origin blocked"

# GET requests should always pass regardless of headers
response=$(vm_exec router "curl -s -k -o /dev/null -w '%{http_code}' -H 'Sec-Fetch-Site: cross-site' https://localhost/login")
