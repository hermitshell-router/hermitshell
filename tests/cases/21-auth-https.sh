...
response=$(vm_exec router "curl -s -k -o /dev/null -w '%{http_code}' -X POST -d 'password=testpass123&confirm=testpass123' https://localhost${setup_action}")
...
response=$(vm_exec router "curl -s -k -o /dev/null -w '%{http_code}' -c /tmp/cookies-test -X POST -d 'password=testpass123' https://localhost${login_action}")
...
response=$(vm_exec router "curl -s -k -o /dev/null -w '%{http_code}' -X POST -d 'password=wrongpass' https://localhost${login_action}")
