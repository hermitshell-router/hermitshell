...
tx_set=$(vagrant ssh router -c "sudo nft list set inet traffic tx_devices" 2>/dev/null || echo "")
assert_contains "$tx_set" "$device_ip" "Device IP in nftables tx_devices counter set"

rx_set=$(vagrant ssh router -c "sudo nft list set inet traffic rx_devices" 2>/dev/null || echo "")
