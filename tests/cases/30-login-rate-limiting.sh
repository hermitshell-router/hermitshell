...
resp=$(vm_sudo router "echo '{\"method\":\"verify_password\",\"value\":\"wrong1\"}' | socat - UNIX-CONNECT:$SOCK")
assert_contains "$resp" '"config_value":"false"' "First wrong attempt not rate-limited"

# Second wrong attempt immediately
resp=$(vm_sudo router "echo '{\"method\":\"verify_password\",\"value\":\"wrong2\"}' | socat - UNIX-CONNECT:$SOCK")
echo "Second attempt: $resp"

# Third attempt should be rate-limited (2s backoff from 2 failures)
resp=$(vm_sudo router "echo '{\"method\":\"verify_password\",\"value\":\"wrong3\"}' | socat - UNIX-CONNECT:$SOCK")
assert_contains "$resp" 'Too many attempts' "Third rapid attempt is rate-limited"

# Correct password also blocked during cooldown
resp=$(vm_sudo router "echo '{\"method\":\"verify_password\",\"value\":\"testpass123\"}' | socat - UNIX-CONNECT:$SOCK")
...
resp=$(vm_sudo router "echo '{\"method\":\"verify_password\",\"value\":\"wrong4\"}' | socat - UNIX-CONNECT:$SOCK")
...
for i in 1 2 3; do
    vm_exec router "curl -s -k -o /dev/null -X POST -d 'password=wrongwebui' https://localhost${login_action}"
done

# Next attempt should get 429
http_code=$(vm_exec router "curl -s -k -o /dev/null -w '%{http_code}' -X POST -d 'password=wrongwebui' https://localhost${login_action}")
