...
deploy_build() {
    ...
    if [ "$HERMIT_MODE" = "deb" ]; then
        (cd "$TESTS_DIR/.." && cargo deb -p hermitshell-agent --no-build)
        cp "$TESTS_DIR"/../target/debian/hermitshell_*.deb "$TESTS_DIR"/../target/release/
        echo "Deb package built: target/release/hermitshell_*.deb"
    fi
}
